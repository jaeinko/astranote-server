...
function buildCoupleDigest(chartA, chartB, nameA, nameB) {
  ...
  /* ── 결혼·동거 지표 ────────────────────────────────────────
     연애 궁합과 결혼 궁합은 보는 자리가 다르다.
     연애는 금성·화성(끌림), 결혼은 4하우스(가정)·7하우스(약속)·토성(지속)이다.
     그래서 "연애 땐 많이 싸웠는데 결혼하니 괜찮다"는 조합이 실제로 존재한다.
     아래는 그 판단에 쓰는 자리만 따로 뽑은 것이다. */
  (function () {
    const M = [];
    const HOME = { 4: '가정·함께 사는 자리', 7: '약속·1:1 파트너십',
                   5: '자녀·즐거움', 2: '생활비·현실 기반', 8: '공동 자산·깊은 결속',
                   10: '사회적 위치·대외적인 우리' };
    /* 상대의 별이 내 어느 방에 들어왔는가 — 결혼과 직결된 방만 추린다 */
    try {
      const ovAll = []
        .concat(houseOverlay(chartA, chartB, nameA, nameB) || [])
        .concat(houseOverlay(chartB, chartA, nameB, nameA) || []);
      ovAll.forEach(function (r) {
        if (!HOME[r.house]) return;
        M.push('· ' + r.text + '  → ' + HOME[r.house]);
      });
    } catch (e) { /* 오버레이가 없으면 건너뛴다 */ }
    /* 토성이 관여하는 각 — 무겁지만 오래 가는 힘 */
    const sat = aspects.filter(function (a) {
      return (a.pA === '토성' || a.pB === '토성');
    });
    if (sat.length) {
      M.push('· 토성이 관여하는 각이 ' + sat.length + '개다. 토성은 관계를 무겁게 만드는 대신 오래 버티게 한다.');
      sat.slice(0, 3).forEach(function (a) { M.push('   - ' + a.text); });
    } else {
      M.push('· 토성이 관여하는 각이 없다. 가볍고 자유롭지만, 붙잡아 두는 힘도 약하다.');
    }
    /* 관계 자체의 파트너십 축 */
    if (comp && comp['상승점']) {
      const cd = signDeg(norm360(comp['상승점'].abs + 180));
      M.push('· 합성차트의 7하우스 시작점: ' + cd.sign + ' ' + cd.deg + '도 — 이 관계가 밖에서 어떤 짝으로 보이는가.');
    }
    if (M.length) {
      L.push('\n[결혼·동거 지표 — 연애 궁합과는 다른 자리를 본다]');
      M.forEach(function (x) { L.push(x); });
      L.push('→ 이 자리들이 채워져 있으면, 연애가 시끄러웠어도 결혼 뒤에 안정되는 조합이다.');
      L.push('  반대로 비어 있으면, 연애는 즐거워도 같이 살기 시작하면 삐걱댄다.');
    }
  })();
