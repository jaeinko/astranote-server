...
// ✅ JSON 파싱 에러(500) 및 타임아웃 방지 로직 적용 완료

// 🔥 [추가] Vercel Pro 플랜 제한 시간 300초 연장 반영
...
나쁜 예(절대 금지): "좋은 사람일 수 있어요", "긍정적인 마음이 중요합니다", "행복한 미래가 기대됩니다" 같은 덕담 금지. 단정적이고 확신에 찬 어조로 써라.

[글 쓰는 방식 - 꼭 지켜]
1. 🚨 [절대 엄수] JSON의 value 값 내부에서는 쌍따옴표(")를 절대 그냥 쓰지 마라. 문장 안에서 문구를 강조하거나 인용할 때는 반드시 홑따옴표(')만 사용해라.
2. 문장 중간에 줄바꿈(Enter)을 하지 말고, 한 줄로 길게 이어 써라. 절대 텍스트 한가운데에서 실제 엔터를 치면 안 된다.
3. 모든 카드에서, 해석을 말하기 전에 반드시 차트상의 근거를 먼저 밝혀라. [차트 근거] → [해석] 순서로 써라.
4. 만나는 시기는 반드시 구체적인 "연도와 월"로 못 박고, 왜 그 시기인지도 차트 근거로 설명하라.
5. 강조할 문장은 <b> 태그로. 마크다운(*) 금지.
6. card2부터 card7까지 항목당 최소 500자 이상 채워라.
...
[출력 JSON 형식]
{
  ...
  "card2_analysis": "(최소 500자) 당신은 어떤 사람인지. 먼저 당신의 태양/달 별자리 위치를 근거로 밝히고, 성향과 연애에서 반복돼온 패턴을 정확하게 짚어라.",
  "card3_appearance": "(최소 500자) 배우자의 외모. 먼저 7하우스의 별자리와 행성을 근거로 밝히고, 그로부터 첫인상/눈빛/체형/분위기/스타일을 생생하게 작성하라.",
  "card4_career": "(최소 500자) 배우자의 직업과 성격. 7하우스와 관련 행성을 근거로 먼저 밝히고, 어떤 일을 하는 사람인지, 돈은 어떻게 쓰는지 구체적으로 서술하라.",
  "card5_timing": "(최소 600자) 만나는 시기를 반드시 구체적인 연도와 월로 못 박고 왜 그 시기인지 행성 흐름을 근거로 설명하라. 어떻게 만나는지와 첫 순간의 상황도 실감나게 작성하라.",
  "card6_chemistry": "(최소 400자) 두 사람의 케미. 두 기운이 만나는 지점을 근거로 밝히고, 어떤 점이 잘 맞고 어떻게 끌리는지를 <b>강조</b> 섞어 서술하라.",
  "card7_destiny_guide": "(최소 500자) 좋은 인연을 잡기 위한 조언. 단, 만나면 힘들어지는 피해야 할 상대의 특징은 <span style='color:#ff3b30;font-weight:900;'>빨간 글씨</span>로 분명하게 경고하라.",
  "card8_teaser": "(더 깊은 리포트를 권하는 미끼 3문장)"
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
          ...
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`,
          {
            ...
            body: JSON.stringify({
              ...
              generationConfig: {
                ...
                temperature: 0.5, // 0.9에서 0.5로 낮추어 형식이 깨질 확률 대폭 다운
        ...
        // 🚨 [방어막 적용] 문자열 내부의 숨은 제어 문자를 공백화하여 JSON 파싱 파괴 현상 차단
