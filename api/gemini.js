...
//
//  ▣ 2026-08-04 v3.1 — '1만자 보장 + 팩폭 양념' 패치
//   1. 검증 하한 70% → 90% (본문 게이트 합계 약 9,540자 + 점수표 설명 ≈ 1만자 보장).
//      프롬프트 목표 자수도 한 단계 상향해 모델이 게이트를 여유 있게 넘도록 함
//      (게이트만 올리면 재생성률이 튀어 토큰비가 샌다).
//   2. 연령대 점수 편차 20점 미만이면 재생성 (기존엔 경고만 찍고 그대로 방출됐음.
//      상세페이지 '전부 비슷한 점수로 뭉개지 않습니다'는 명시 약속이다).
//   3. 약속 키워드 게이트 확대 — card2 '절대 손대면 안 되는',
//      card4 '익숙한 길'·'단 하나의 숙제'.
//   4. life_desc_10~80 누락 검증 추가 (점수만 있고 설명이 빈 채 렌더되던 구멍).
//   5. [💥 팩트폭력 규격] 신설 — 배치 근거 있는 뼈 때리는 단정을 챕터당 1개 이상.
//      배치→팩폭 매핑을 명시해 근거 없는 팩폭(할루시네이션)을 차단.
//   6. 화자 경력 '이십 년' → '십 년 넘게' (브랜드 서사와 일치).
// ============================================================================
...
function buildCommonPrompt(v) {
  return `
[화자 설정 — 이 목소리로만 써라]
너는 명리학(사주)을 십 년 넘게 공부한 뒤 서양 점성술로 옮겨온 상담가다. 이과 출신이라 근거 없는 말을 싫어한다.
...
각 챕터에 최소 하나, 읽는 순간 "어떻게 알았지" 소리가 나오는 뼈 때리는 단정을 넣어라. 단:
① 반드시 위 차트에 실제로 있는 배치에서만 도출하라. 근거 없는 팩폭은 치명적 실패다. 예시 매핑 —
   · 달-토성 긴장 / 토성 4하우스 / 달이 염소·12하우스 → 억눌린 가정. "집에 사랑이 없던 게 아닙니다. 사랑에 조건이 붙어 있었죠. 잘해야 예뻐하는 집이었습니다."
   · 달·금성이 목성과 조화인데 달-천왕성·명왕성 긴장 동반 → 채워지지 않는 갈증. "부족한 것 없이 자랐는데도 늘 허기가 졌을 겁니다. 받은 사랑의 문제가 아니라, 자극 없이는 못 견디는 배치라 그렇습니다."
   · 화성-해왕성 긴장 / 목성-해왕성 긴장 / 2·8하우스에 해왕성 / 화성 함몰·추락 → 사업·투기 경고. "지금 이대로 사업하면 안 됩니다. 능력이 없어서가 아니라, 숫자가 흐려지는 배치이기 때문입니다."
   · 금성-명왕성 긴장 → "사랑과 소유를 구분 못 해서 관계를 통째로 태워먹은 적이 있으실 겁니다."
   · 달-천왕성 긴장 → "안정을 원한다고 말하면서, 막상 안정이 오면 제일 먼저 지루해하는 건 본인입니다."
② 팩폭과 근거(도·분 또는 각도)는 반드시 같은 단락 안에 붙여 써라. 근거 없이 던지는 순간 점쟁이 사기가 된다.
③ 팩폭으로 끝내지 마라. 반드시 '그래서 그 기질을 어떻게 쓰면 무기가 되는지'로 닫아라(공감→반전 규칙과 동일).
④ '~하면 안 됩니다' 류 금지 팩폭은 리포트 전체에서 최대 두 개. 해당 배치가 없으면 아예 쓰지 마라. 셋 이상이면 겁주기 장사로 읽힌다.
...
const PROMISE_KEYWORDS = {
  vip_card1: ['가짜 방어기제'],
  vip_card2: ['절대 손대면 안 되는'],
  vip_card3: ['골든 크로스', '대운', '귀인', '악연'],
  vip_card4: ['익숙한 길', '단 하나의 숙제']
};
...
const VALIDATE_A = makeValidator([
  ...
  { key: 'vip_card1', min: 2160 },   // 원목표 2400의 90%. 프롬프트 목표는 2600으로 상향해 여유 확보
  { key: 'vip_card2', min: 2880 }    // 원목표 3200의 90%. 프롬프트 목표 3400
]);
const VALIDATE_B_BASE = makeValidator([
  { key: 'vip_card3', min: 2160 },   // 원목표 2400의 90%. 프롬프트 목표 2600
  { key: 'vip_card4', min: 1980 },   // 원목표 2200의 90%. 프롬프트 목표 2400
  { key: 'closing', min: 360 },      // 원목표 400의 90%
  { key: 'best_age' }, { key: 'best_age_reason', min: 60 },
  ...
  // 🚨 v3.1 — 점수만 있고 설명이 빈 채 렌더되던 구멍. 누락은 관용 모드에서도 차단된다.
  { key: 'life_desc_10', min: 20 }, { key: 'life_desc_20', min: 20 },
  { key: 'life_desc_30', min: 20 }, { key: 'life_desc_40', min: 20 },
  { key: 'life_desc_50', min: 20 }, { key: 'life_desc_60', min: 20 },
  { key: 'life_desc_70', min: 20 }, { key: 'life_desc_80', min: 20 }
]);

/* 🚨 v3.1 — 연령대 점수 편차 강제
   상세페이지가 "전부 비슷한 점수로 뭉개지 않습니다"를 명시적으로 약속했다.
   기존에는 normalizeScores 에서 경고만 찍고 그대로 방출됐다 = 약속 불이행이 새어나감.
   여기서 재생성을 태운다. 단, 마지막 시도(관용 모드)에서는 손님에게 500을 주는 것보다
   편차 부족 상태로라도 방출하는 편이 낫므로 경고만 남긴다. */
function VALIDATE_B(d, lenient) {
  const base = VALIDATE_B_BASE(d, lenient);
  if (!base.ok) return base;
  const s = [10, 20, 30, 40, 50, 60, 70, 80].map(x => Number(d['life_score_' + x]));
  if (s.every(n => !isNaN(n))) {
    const spread = Math.max.apply(null, s) - Math.min.apply(null, s);
    if (spread < 20) {
      if (!lenient) return { ok: false, reason: '연령대 점수 편차 ' + spread + '점 < 20점 — 뭉개진 점수표는 상세페이지 약속 위반' };
      console.warn('⚠️ [편차 부족 방출] ' + spread + '점');
    }
  }
  return { ok: true };
}
...
function normalizeScores(d) {
  ...
  // v3.1 소프트 체크 — 상세페이지 약속 "실제 연도까지 함께 짚어드립니다"
  if (d.best_age_reason && !/\d{4}\s*년/.test(String(d.best_age_reason)))
    console.warn('⚠️ best_age_reason에 4자리 연도 없음 — 약속 이행 여부 확인 필요');
  return d;
}
