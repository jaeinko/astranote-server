...
const handler = async (req, res) => {
  ...
  try {
    ...
    const BANNED = ['undefined', 'null', 'NaN', '트랜짓 항목', '데이터에 없음',
      ...
      '긍정적으로 생각', '시간이 해결'];
    const NEED_LEN = {
      card2_analysis: 550, card3_appearance: 380, card4_career: 380,
      card5_timing: 550, card6_chemistry: 380, card7_destiny_guide: 450
    };
...
너는 명리학을 이십 년 공부한 뒤 서양 점성술로 옮겨온 상담가다.
...
[출력 JSON]
{
  ...
  "card4_career": "(500자 이상) 그 사람의 사회적 위치와 경제적 결. 7하우스 지배성이 있는 하우스를 근거로 직업의 결을 짚어라. 두루뭉술한 '전문직'이 아니라 어떤 방식으로 일하는 사람인지를 써라. 돈을 대하는 태도도 함께.",

  "card5_timing": "(700자 이상) ★두 번째로 중요★ 만남의 시기와 자리. [실제 계산된 목성 트랜짓]의 연·월을 그대로 인용하고, 왜 그 시기인지 근거를 밝혀라. 그리고 7하우스 지배성이 있는 하우스를 근거로 '어디서 만나는지'를 구체적으로 짚어라. 시기가 계산되지 않았으면 정직하게 말하고 자리 쪽에 집중하라.",
