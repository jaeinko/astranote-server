...
const SPEC = [
  { key: 'ch1_nature',    min: 2400 },
  { key: 'ch2_inside',    min: 2400 },
  { key: 'ch3_outside',   min: 1600 },
  { key: 'ch4_talent',    min: 1800 },
  { key: 'ch5_chemistry', min: 2400, needsParent: true },
  { key: 'ch6_pace',      min: 1800 },
  { key: 'ch7_love',      min: 1600 },
  { key: 'ch8_timeline',  min: 2400 },
  { key: 'ch9_tenyears',  min: 1400 },
  { key: 'closing',       min: 400 }
];
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const bad = validate(candidate, P, C, built.hasParent, attempt === 3);
        if (bad) {
          lastErr = bad;
          correction = bad;
