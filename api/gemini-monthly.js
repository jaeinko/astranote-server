...
🚨 마크다운 금지. **굵게**, *기울임*, # 제목 전부 안 된다. 이 글은 HTML로 출력되므로 별표가 화면에 그대로 보인다.
   강조는 <b>강조할 말</b> 만 써라.
🚨 '오차 0.3도', '트랜짓 화성 — 내 태양 삼각' 같은 계산 표기를 그대로 옮기지 마라. 손님에게는 암호로 보인다.
   그 각이 뜻하는 '실제 벌어지는 일'로 풀어써라.
...
[🚨 점수 규칙]
위에 '확정 점수'로 주어진 숫자를 단 1점도 바꾸지 마라. 점수를 나열만 하지 말고, 왜 그 점수인지를 실제 흐름으로 설명하라.
...
[출력 형식 — 아래 JSON 키를 정확히 그대로, 순수 JSON만]
{
  ...
  "card8_action": "(350자 이상) 이 30일 동안 해야 할 것 3가지와 하지 말아야 할 것 3가지. 행동 단위로. '긍정적으로 생각하세요' 같은 뻔한 조언 금지.",
...
const BANNED = /undefined|null|NaN|트랜짓 목록|다이제스트|확정 점수|정보 완전도|오차 \d/i;
